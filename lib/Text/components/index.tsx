    ...
    if (options?.inputStyle) {
      return (
        <div draggable={false} contentEditable={editable} className="font-sans dark:tracking-wide">
          {children}
          <CharacterCount element={element} type='body' countTypes={options?.countCharacters as string[]} />
        </div>
      )
    }
  ...
  switch (style?.role) {
    case 'heading-1':
      return (
        <div draggable={false} contentEditable={editable} className="font-sans font-bold text-4xl pt-2 pb-4">
          {children}
          <CharacterCount element={element} type='heading-1' countTypes={options?.countCharacters as string[]} />
        </div>
      )
    ...
    case 'heading-2':
      return <div draggable={false} contentEditable={editable} className="font-sans font-bold text-xl py-2">
        {children}
        <CharacterCount element={element} type='heading-2' countTypes={options?.countCharacters as string[]} />
      </div>

    case 'preamble':
      return <div draggable={false} contentEditable={editable} className="font-serif font-semibold py-2 dark:tracking-wide">
        {children}
        <CharacterCount element={element} type='preamble' countTypes={options?.countCharacters as string[]} />
      </div>
    ...
    case 'vignette':
      return <div draggable={false} contentEditable={editable} className="font-sans font-bold py-1 text-sm uppercase">
        ...
        <CharacterCount element={element} type='vignette' className='normal-case' countTypes={options?.countCharacters as string[]} />
      </div>
    ...
    default:
      return <div draggable={false} contentEditable={editable} className="font-serif opacity-80 italic line-through px-1 bg-red-200 dark:text-gray-700">
        {children}
        <CharacterCount element={element} type='default' countTypes={options?.countCharacters as string[]} />
      </div>
  }
}
